...
2. A Distribuição de Poisson possui uma propriedade chamada equidispersão: a média e a variância são iguais ao parâmetro $\lambda$. Os valores calculados para `Richness` estão próximos entre si?
...
ggplot(rikz, aes(x = NAP_c, y = Richness)) +
  geom_point(alpha = 0.6, color = "#1a9988", size = 3) +
  labs(
...
3. A Distribuição de Poisson tem variância igual à média: $\text{Var}[Y] = E[Y] = \lambda$. Como essa propriedade se manifesta na largura do intervalo preditivo ao longo da curva?
...
::: {.callout-tip title="Dica"}
`posterior_epred()` captura apenas a incerteza sobre a curva média ($\beta_0$ e $\beta_1$). `posterior_predict()` acrescenta a variabilidade individual das contagens. Compare a largura do intervalo preditivo nas regiões onde a curva média é mais alta e onde é mais baixa: a largura permanece constante ou varia ao longo de `NAP_c`? Relacione o que você observa com a propriedade da Distribuição de Poisson que vincula a variância à taxa esperada $\lambda_i$.
:::
