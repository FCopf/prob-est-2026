...
3. Na leitura prévia, a relação altura-peso tinha direção positiva claramente esperada. Neste contexto ecológico, o sinal de $\beta_1$ é igualmente previsível? O que o diagrama sugere?
...
1. Na leitura prévia, a distribuição a priori de $\beta_1$ foi $\text{Lognormal}(0, 1)$. Por que essa mesma escolha seria inadequada para o coeficiente de `NAP_c` neste contexto ecológico?
...
3. O que acontece com o feixe quando você aumenta bastante o desvio padrão da distribuição a priori de $\beta_1$? Quando você o diminui muito?
...
ggplot(rikz, aes(x = NAP_c, y = Richness)) +
  geom_point(alpha = 0.15, color = "#1a9988") +
  geom_abline(
...
::: {.callout-tip title="Dica"}
A Distribuição Lognormal é definida apenas para valores positivos. Se o coeficiente de regressão pode ser positivo ou negativo, uma distribuição que cobre ambos os lados do zero é mais adequada. Para o intercepto $\beta_0$, pense em qual é a riqueza esperada em uma estação com NAP igual à média: valores entre 5 e 15 espécies seriam plausíveis para praias costeiras temperadas.
:::
...
::: {.callout-tip title="Dica"}
O argumento `sample_prior = "only"` foi o que distinguia a checagem a priori do ajuste completo. Sem ele, o `brm()` usa os dados observados para atualizar as distribuições a priori e gerar a distribuição a posteriori. É exatamente a atualização de crenças descrita pela regra de Bayes: a distribuição a posteriori é proporcional à verossimilhança multiplicada pelas distribuições a priori.
:::
...
2. Calcule o resumo com `summarise_draws()`, usando a média, o desvio padrão e os quantis 5,5% e 94,5% (intervalo de credibilidade de 89%).
...
2. Qual é o sinal da média a posteriori de $\beta_1$? O intervalo de credibilidade de 89% inclui zero ou exclui completamente um dos lados? O que isso indica sobre a relação NAP-riqueza?
...
# Resumo: média, desvio padrão e intervalo de credibilidade de 89%
summarise_draws(
  amostras_rikz,
  mean, sd,
...
3. Calcule os resumos (média e quantis 5,5% e 94,5%) para cada posição de predição com `apply()`.
...
resumo_epred <- data.frame(
  ...
  q_inf = apply(epred_rikz, 2, quantile, 0.055),
  q_sup = apply(epred_rikz, 2, quantile, 0.945)
)

resumo_pred <- data.frame(
  NAP_c = novos_dados$NAP_c,
  q_inf = apply(pred_rikz, 2, quantile, 0.055),
  q_sup = apply(pred_rikz, 2, quantile, 0.945)
)
...
::: {.callout-tip title="Dica"}
`posterior_epred()` captura somente a incerteza sobre $\beta_0$ e $\beta_1$: onde está a reta. `posterior_predict()` acrescenta a variabilidade individual capturada por $\sigma$: onde cairia uma nova observação. O intervalo preditivo é sempre mais largo porque inclui essa fonte adicional de variabilidade. A função usada determina o que está sendo estimado: a reta média ou uma observação futura.
:::
