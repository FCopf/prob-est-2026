...
$$\text{AFD} = a \cdot \text{comprimento}^b$$

em que $a$ é um coeficiente de escala e $b$ é o coeficiente alométrico. Na escala original, essa equação gera uma curva com crescimento acelerado. Visualizar a relação nessa escala dificulta a avaliação da força e da regularidade da associação.

::: {.callout-note appearance="minimal" title="Definição: coeficiente alométrico"}
O **coeficiente alométrico** $b$ descreve como uma medida biológica escala com o tamanho do organismo. Quando $b = 3$, a biomassa cresce isometricamente com o cubo do comprimento linear (crescimento isométrico). Quando $b > 3$, o crescimento relativo da biomassa supera o esperado pela isometria (alometria positiva). Quando $b < 3$, o crescimento relativo da biomassa é inferior ao esperado (alometria negativa).
:::
...
$$\ln(\text{AFD}) = \ln(a) + b \cdot \ln(\text{comprimento})$$

A relação de potência se converte em uma equação linear na escala logarítmica. O coeficiente $b$ torna-se a inclinação da reta ajustada no gráfico log-log, e $\ln(a)$ é o intercepto. Um gráfico de $\ln(\text{AFD})$ contra $\ln(\text{comprimento})$ deve mostrar uma nuvem de pontos próxima de uma linha reta se a relação de potência descrever adequadamente os dados.
...
::: {.callout-tip appearance="minimal" title="O que observar"}
Na escala logarítmica, os pontos se distribuem em torno de uma linha reta e a dispersão parece mais uniforme ao longo do eixo $x$ do que no gráfico original. A inclinação da reta ajustada é uma estimativa visual do coeficiente alométrico $b$.
:::
